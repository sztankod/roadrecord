...
class RoadRecordRepository(private val dao:RoadRecordDao){
 ...
 suspend fun startWork(now:Long=System.currentTimeMillis()):Long {ensureDefaults();val existing=activeDay();if(existing!=null)return existing.day.id;val previous=dao.latestClosedDay();val p=dao.activePeriod()?:error("Nincs aktív időszak");val date=Instant.ofEpochMilli(now).atZone(java.time.ZoneId.systemDefault()).toLocalDate().toString();val id=dao.insertDay(WorkDay(periodId=p.id,date=date));dao.insertEvent(WorkEvent(workDayId=id,type=EventType.WORK_START,timestamp=now));previous?.let{old->dao.plansNow(old.day.id).forEach{dao.upsertPlan(it.copy(workDayId=id,visited=false))};dao.routeConfigNow(old.day.id)?.let{dao.saveRouteConfig(it.copy(workDayId=id))}};return id}
 ...
 suspend fun recognizePlannedStops(dayId:Long,latitude:Double,longitude:Double,time:Long,accuracy:Float){var current:Long?=null;dao.plansNow(dayId).forEach{plan->val place=dao.place(plan.placeId)?:return@forEach;val lat=place.latitude?:return@forEach;val lon=place.longitude?:return@forEach;val earth=6371000.0;val dLat=Math.toRadians(lat-latitude);val dLon=Math.toRadians(lon-longitude);val a=kotlin.math.sin(dLat/2).let{it*it}+kotlin.math.cos(Math.toRadians(latitude))*kotlin.math.cos(Math.toRadians(lat))*kotlin.math.sin(dLon/2).let{it*it};val distance=2*earth*kotlin.math.atan2(kotlin.math.sqrt(a),kotlin.math.sqrt(1-a));if(distance<=place.recognitionRadiusMeters+accuracy.coerceAtMost(50f)){current=place.id;if(!plan.visited){dao.setPlanVisited(dayId,place.id,true);if(dao.visitCount(dayId,place.id)==0)dao.insertVisit(PlaceVisit(workDayId=dayId,placeId=place.id,arrivalTime=time,distanceMeters=distance))}}};dao.settings()?.let{if(it.currentPlaceId!=current)dao.saveSettings(it.copy(currentPlaceId=current))}}
