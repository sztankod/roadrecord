...
class RoadRecordRepository(private val dao:RoadRecordDao){
 ...
 suspend fun recognizePlannedStops(dayId:Long,latitude:Double,longitude:Double,time:Long,accuracy:Float){dao.plansNow(dayId).filter{!it.visited}.forEach{plan->val place=dao.place(plan.placeId)?:return@forEach;val lat=place.latitude?:return@forEach;val lon=place.longitude?:return@forEach;val earth=6371000.0;val dLat=Math.toRadians(lat-latitude);val dLon=Math.toRadians(lon-longitude);val a=kotlin.math.sin(dLat/2).let{it*it}+kotlin.math.cos(Math.toRadians(latitude))*kotlin.math.cos(Math.toRadians(lat))*kotlin.math.sin(dLon/2).let{it*it};val distance=2*earth*kotlin.math.atan2(kotlin.math.sqrt(a),kotlin.math.sqrt(1-a));if(distance<=place.recognitionRadiusMeters+accuracy.coerceAtMost(50f)){dao.setPlanVisited(dayId,place.id,true);if(dao.visitCount(dayId,place.id)==0)dao.insertVisit(PlaceVisit(workDayId=dayId,placeId=place.id,arrivalTime=time,distanceMeters=distance))}}}
