...
@Dao interface RoadRecordDao {
 ...
 @Query("UPDATE daily_place_plans SET sortHint=:sortHint, lockedPosition=:lockedPosition WHERE workDayId=:dayId AND placeId=:placeId") suspend fun updatePlanPosition(dayId:Long,placeId:Long,sortHint:Int?,lockedPosition:Int?)
 @Transaction suspend fun updatePlanPositions(plans:List<DailyPlacePlan>){plans.forEach{updatePlanPosition(it.workDayId,it.placeId,it.sortHint,it.lockedPosition)}}
 ...
 @Query("UPDATE daily_place_plans SET recognitionDiagnostic=:diagnostic WHERE workDayId=:dayId AND placeId=:placeId") suspend fun recordRecognitionDiagnostic(dayId:Long,placeId:Long,diagnostic:String)
