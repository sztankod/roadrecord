...
class TrackingService:Service(){
 ...
 private val scope=CoroutineScope(SupervisorJob()+Dispatchers.IO);private lateinit var client:FusedLocationProviderClient;private var dayId=0L;private var bakeryInside:Boolean?=null
 private val callback=object:LocationCallback(){override fun onLocationResult(result:LocationResult){val l=result.lastLocation?:return;scope.launch{val app=application as RoadRecordApplication;val inside=app.repository.bakeryPresence(l.latitude,l.longitude,l.accuracy);if(inside!=null){val previous=bakeryInside;if(previous!=null&&previous!=inside)app.repository.automaticBakeryTransition(dayId,left=!inside,time=l.time)else if(previous==null&&inside&&app.repository.activeTrip(dayId)!=null)app.repository.automaticBakeryTransition(dayId,left=false,time=l.time);bakeryInside=inside};app.repository.recognizePlannedStops(dayId,l.latitude,l.longitude,l.time,l.accuracy);val trip=app.repository.activeTrip(dayId)?:return@launch;app.repository.addGpsPoint(GpsPoint(tripId=trip.id,timestamp=l.time,latitude=l.latitude,longitude=l.longitude,accuracy=l.accuracy))}}}
