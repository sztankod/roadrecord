...
class TrackingService:Service(){
 companion object { const val ACTION_START="hu.roadrecord.START";const val ACTION_STOP="hu.roadrecord.STOP";const val EXTRA_DAY="day";private const val CHANNEL="active_work";private const val ARRIVAL_CHANNEL="arrival_warning";private const val ID=77 }
 ...
 private fun warnGps(name:String){val n=NotificationCompat.Builder(this,ARRIVAL_CHANNEL).setSmallIcon(android.R.drawable.ic_dialog_alert).setContentTitle("GPS-koordináta pontosítása szükséges").setContentText("$name koordinátája még nincs manuálisan pontosítva.").setAutoCancel(true).setDefaults(NotificationCompat.DEFAULT_SOUND or NotificationCompat.DEFAULT_VIBRATE).build();NotificationManagerCompat.from(this).notify((name.hashCode() and 0x7fffffff)+100,n)}
