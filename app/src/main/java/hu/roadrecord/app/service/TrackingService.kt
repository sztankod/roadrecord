...
class TrackingService:Service(){
 ...
 private val scope=CoroutineScope(SupervisorJob()+Dispatchers.IO);private lateinit var client:FusedLocationProviderClient;private var dayId=0L;private var bakeryInside:Boolean?=null;private var currentStopId:Long?=null;private var pendingStopId:Long?=null;private var pendingStopSince:Long?=null;private var outsideHits=0
 private val callback=object:LocationCallback(){override fun onLocationResult(result:LocationResult){val l=result.lastLocation?:return;scope.launch{
  ...
  val detection=app.repository.detectPlannedStop(dayId,l.latitude,l.longitude,l.accuracy);val detectedId=detection?.place?.id
  if(detectedId!=null){outsideHits=0;app.repository.previewCurrentStop(detectedId,detection.distanceMeters);if(pendingStopId!=detectedId){pendingStopId=detectedId;pendingStopSince=l.time};val stayed=(l.time-(pendingStopSince?:l.time)).coerceAtLeast(0);if(currentStopId!=detectedId&&stayed>=app.repository.automaticVisitDelayMillis()){val stop=app.repository.applyStopDetection(dayId,detection,l.time);currentStopId=stop?.id;if(stop!=null&&!stop.gpsManuallyConfirmed)playCoordinateWarning()}}
  else{pendingStopId=null;pendingStopSince=null;outsideHits++;if(outsideHits>=2&&currentStopId!=null){app.repository.applyStopDetection(dayId,null,l.time);app.repository.previewCurrentStop(null,null);currentStopId=null}}
 ...
 override fun onStartCommand(intent:Intent?,flags:Int,startId:Int):Int{if(intent?.action==ACTION_STOP){stopTracking();return START_NOT_STICKY};intent?.getLongExtra(EXTRA_DAY,0)?.takeIf{it>0}?.let{if(dayId!=it){bakeryInside=null;currentStopId=null;pendingStopId=null;pendingStopSince=null;outsideHits=0};dayId=it};startForeground(ID,notification());requestLocation();return START_STICKY}
