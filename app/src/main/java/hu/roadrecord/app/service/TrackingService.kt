...
class TrackingService:Service(){
 ...
 private val scope=CoroutineScope(SupervisorJob()+Dispatchers.IO);private lateinit var client:FusedLocationProviderClient;private var dayId=0L;private val bakeryTripGate=BakeryTripGate();private var locationRequested=false;private var currentStopId:Long?=null;private var pendingStopId:Long?=null;private var pendingStopSince:Long?=null;private var pendingMisses=0;private var currentOutsideHits=0
 ...
 val app=application as RoadRecordApplication
  ...
  val inside=app.repository.bakeryPresence(l.latitude,l.longitude,l.accuracy);if(inside!=null){val active=app.repository.activeTrip(dayId)!=null;when(bakeryTripGate.observe(inside,active)){BakeryTripAction.START->app.repository.automaticBakeryTransition(dayId,left=true,time=l.time);BakeryTripAction.CLOSE->app.repository.automaticBakeryTransition(dayId,left=false,time=l.time);BakeryTripAction.NONE->Unit}}
 ...
 override fun onStartCommand(intent:Intent?,flags:Int,startId:Int):Int{if(intent?.action==ACTION_STOP){stopTracking();return START_NOT_STICKY};startForeground(ID,notification());val requestedDay=intent?.getLongExtra(EXTRA_DAY,0)?.takeIf{it>0};if(requestedDay!=null){selectDay(requestedDay);requestLocation()}else if(dayId>0){requestLocation()}else scope.launch{val restored=(application as RoadRecordApplication).repository.activeDay()?.day?.id;if(restored!=null){selectDay(restored);withContext(Dispatchers.Main){requestLocation()}}else withContext(Dispatchers.Main){stopTracking()}};return START_STICKY}
 ...
 private fun selectDay(id:Long){if(dayId==id)return;dayId=id;bakeryTripGate.reset();currentStopId=null;pendingStopId=null;pendingStopSince=null;pendingMisses=0;currentOutsideHits=0}
 private fun requestLocation(){if(locationRequested||ActivityCompat.checkSelfPermission(this,Manifest.permission.ACCESS_FINE_LOCATION)!=PackageManager.PERMISSION_GRANTED)return;val request=LocationRequest.Builder(Priority.PRIORITY_HIGH_ACCURACY,8_000).setMinUpdateIntervalMillis(5_000).setMinUpdateDistanceMeters(0f).build();locationRequested=true;client.requestLocationUpdates(request,callback,mainLooper).addOnFailureListener{locationRequested=false}}
 private fun stopTracking(){locationRequested=false;client.removeLocationUpdates(callback);stopForeground(STOP_FOREGROUND_REMOVE);stopSelf()}
