...
class TrackingService:Service(){
 ...
 private val scope=CoroutineScope(SupervisorJob()+Dispatchers.IO);private lateinit var client:FusedLocationProviderClient;private var dayId=0L;private var bakeryInside:Boolean?=null;private var currentStopId:Long?=null;private var pendingStopId:Long?=null;private var pendingStopInitialized=false;private var pendingStopHits=0
 private val callback=object:LocationCallback(){override fun onLocationResult(result:LocationResult){val l=result.lastLocation?:return;scope.launch{val app=application as RoadRecordApplication;val inside=app.repository.bakeryPresence(l.latitude,l.longitude,l.accuracy);if(inside!=null){val previous=bakeryInside;if(previous!=null&&previous!=inside)app.repository.automaticBakeryTransition(dayId,left=!inside,time=l.time)else if(previous==null&&inside&&app.repository.activeTrip(dayId)!=null)app.repository.automaticBakeryTransition(dayId,left=false,time=l.time);bakeryInside=inside};val detection=app.repository.detectPlannedStop(dayId,l.latitude,l.longitude,l.accuracy);val detectedId=detection?.place?.id;if(!pendingStopInitialized||pendingStopId!=detectedId){pendingStopId=detectedId;pendingStopHits=1;pendingStopInitialized=true;if(detectedId!=null)app.repository.previewCurrentStop(detectedId)}else pendingStopHits++;if(pendingStopHits>=2&&currentStopId!=detectedId){val stop=app.repository.applyStopDetection(dayId,detection,l.time);currentStopId=stop?.id;if(stop!=null&&!stop.gpsManuallyConfirmed)playCoordinateWarning()};if(NextStopOverlayService.visible){val next=app.repository.nextPlannedStop(dayId,detectedId);startService(Intent(this@TrackingService,NextStopOverlayService::class.java).setAction(NextStopOverlayService.ACTION_UPDATE).putExtra(NextStopOverlayService.EXTRA_CURRENT_NAME,detection?.place?.name?:"Úton").putExtra(NextStopOverlayService.EXTRA_CURRENT_ADDRESS,detection?.place?.officialAddress.orEmpty()).putExtra(NextStopOverlayService.EXTRA_NAME,next?.name?:"Nincs további megálló").putExtra(NextStopOverlayService.EXTRA_ADDRESS,next?.officialAddress.orEmpty()))};val trip=app.repository.activeTrip(dayId)?:return@launch;app.repository.addGpsPoint(GpsPoint(tripId=trip.id,timestamp=l.time,latitude=l.latitude,longitude=l.longitude,accuracy=l.accuracy))}}}
 ...
 override fun onStartCommand(intent:Intent?,flags:Int,startId:Int):Int{if(intent?.action==ACTION_STOP){stopTracking();return START_NOT_STICKY};intent?.getLongExtra(EXTRA_DAY,0)?.takeIf{it>0}?.let{if(dayId!=it){bakeryInside=null;currentStopId=null;pendingStopId=null;pendingStopInitialized=false;pendingStopHits=0};dayId=it};startForeground(ID,notification());requestLocation();return START_STICKY}
